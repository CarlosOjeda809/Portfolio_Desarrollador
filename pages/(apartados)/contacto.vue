...
<template>
  <main class="container mx-auto p-4 md:px-16 lg:px-24 xl:p-4">
    <section
      class="bg-gray-50/60 border-2 border-gray-400/20 backdrop-blur-lg rounded-3xl shadow-lg p-10 mt-30 relative z-10"
      aria-labelledby="contacto-titulo"
    >
      <h2 id="contacto-titulo" class="text-4xl font-bold text-purple-600 mb-10 text-center">
        ¡Hablemos!
      </h2>
      <form @submit.prevent="enviarFormulario" class="grid sm:grid-cols-2 gap-8 relative z-20 text-gray-700" novalidate>
        <fieldset>
          <label for="nombre" class="block text-gray-800 text-lg font-semibold mb-2">Nombre *</label>
          <input
            type="text"
            id="nombre"
            v-model="nombre"
        ...
        <fieldset>
          <label for="email" class="block text-gray-800 text-lg font-semibold mb-2">Email *</label>
          <input
            type="email"
            id="email"
            v-model="email"
        ...
        <fieldset>
          <label for="asunto" class="block text-gray-800 text-lg font-semibold mb-2">Asunto</label>
          <input
            type="text"
            id="asunto"
            v-model="asunto"
        ...
        <fieldset>
          <label for="telefono" class="block text-gray-800 text-lg font-semibold mb-2">Teléfono</label>
          <input
            type="tel"
            id="telefono"
            v-model="telefonoData"
            class="shadow-sm focus:ring-purple-500 border focus:border-purple-500 block w-full sm:text-lg border-gray-300 rounded-md py-3 px-4 text-gray-800"
          />
        </fieldset>

        <fieldset class="col-span-2">
          <label for="mensaje" class="block text-gray-800 text-lg font-semibold mb-2">Mensaje</label>
          <textarea
            id="mensaje"
            rows="5"
            v-model="mensaje"
        ...
        <fieldset class="col-span-2 hidden">
          <label for="ubicacion" class="block text-gray-800 text-lg font-semibold mb-2">Ubicación</label>
          <input
            type="text"
            id="ubicacion"
            v-model="ubicacion"
        ...
        <fieldset class="col-span-2 flex justify-center">
          <button
            type="submit"
          ...
          >
            <div class="flex items-center">
              ...
              <div v-if="!enviando" class="ml-3">
                <Icon name="material-symbols:send" class="text-xl mt-0.5" />
              </div>
            </div>
          </button>
        </fieldset>
        ...
        <div
          v-if="mensajeEstado"
          role="alert"
          class="text-center p-4 justify-center col-span-2 rounded-md text-lg"
